...
interface Step {
  id: string;
  section: string;
  type: 'select' | 'multiselect' | 'text' | 'numberGrid' | 'statGrid' | 'interstitial' | 'wheel';
  question?: string;
  subtitle?: string;
  options?: OptionDef[];
  placeholder?: string;
...
const STEPS: Step[] = [
  ...
  { id: 'age', section: 'About You', type: 'wheel', wheelKind: 'age', question: 'How old are you?', subtitle: 'Helps me pick the right intensity for you.' },
  { id: 'height', section: 'About You', type: 'wheel', wheelKind: 'height', question: 'How tall are you?', subtitle: 'Affects which finishing and defense moves fit your game.' },
  { id: 'weight', section: 'About You', type: 'wheel', wheelKind: 'weight', question: 'How much do you weigh?', subtitle: 'Helps me dial in conditioning and contact drills.' },
  { id: 'description', section: 'About You', type: 'text', question: 'Describe yourself.', subtitle: "Jersey color, hair, anything so Coach X knows who you are.", placeholder: 'e.g. red jersey #5, curly hair' },
...
export default function TodayScreen() {
  ...
  const goNext = () => {
    ...
    if (stepIndex < updatedVisible.length - 1) {
      ...
      // ===== CHANGED: instead of going to auth, go to readback =====
      startReadback();
    }
  };
  ...
  // ===== NEW: wheel picker change =====
  const handleWheelChange = (id: string, value: string) => {
  ...
  const startReadback = async () => {
    ...
    try {
      ...
      if (r.ok && data.readback) {
        ...
        setReadbackFocus(data.suggestedFocus || payload.weakness || '');
      } else {
        setReadbackText(buildFallbackReadback());
        setReadbackFocus(payload.weakness || 'Ball handling');
      }
    } catch (e) {
      setReadbackText(buildFallbackReadback());
      setReadbackFocus('Ball handling');
    }
  ...
  const buildFallbackReadback = () => {
    ...
    const pos = answers.position || 'player';
    return "Alright. " + age + " year old " + pos + ". I got a read on your game. Sound right?";
  };
  ...
  const handleFocusConfirm = () => {
    if (Platform.OS !== 'web') void Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
    setAnswers({ ...answers, chosenFocus });
    setAppState('auth');
  };
  ...
  const debugJumpToClimax = () => {
    ...
    usePlanStore.getState().setProfile({
      sport: 'Basketball', position: 'Point Guard', experience: '3-5 years',
      goal: 'Become a better all-around scorer', weakness: 'Weak hand finishing',
      frequency: '3-4 times a week', duration: '30-45 minutes', access: ['Half court with hoop'],
      driving: 'not specified', leftHand: 'Weak - I avoid it', pressure: 'not specified',
      goToMove: 'not specified', threeConfidence: 'Somewhat', freeThrow: '60-80%',
    } as any);
    setAppState('climax');
  };
  ...
  const buildPayloadFromAnswers = (a: Record<string, any>) => {
    ...
    // ===== NEW: if user picked a focus override, use that =====
    if (a.chosenFocus) weakness = a.chosenFocus;
      ...
      <View style={[s.c, { paddingTop: insets.top, paddingBottom: insets.bottom }]}>
        <ScrollView contentContainerStyle={{ paddingHorizontal: 24, paddingTop: 20, paddingBottom: 140 }} showsVerticalScrollIndicator={false}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 24 }}>
            ...
            <View>
              <Text style={{ fontSize: 11, fontWeight: '700', color: Colors.primary, letterSpacing: 1.5, marginBottom: 2 }}>COACH X</Text>
              <Text style={{ fontSize: 13, color: Colors.textMuted }}>Picking your focus</Text>
            </View>
          </View>
          ...
          {FOCUS_OPTIONS.map((opt) => {
            ...
            return (
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <Text style={[s.optTxt, sel && s.optTxtSel, { flex: 1 }]}>{opt.label}</Text>
        ...
        <View style={s.bn}>
          <TouchableOpacity
            style={[s.cb, !chosenFocus && s.cbDisabled]}
            onPress={handleFocusConfirm}
            activeOpacity={0.85}
            disabled={!chosenFocus}
          >
            <Text style={[s.ct, !chosenFocus && s.ctDisabled]}>Let's go</Text>
          </TouchableOpacity>
        </View>
      </View>
    );
