  ...
  return (
    <View style={[s.container, { paddingTop: insets.top, paddingBottom: insets.bottom }]}>
      ...
      <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        <Text style={s.subtitle}>{mode === 'signup' ? 'Save your progress and training plan.' : 'Sign in to continue training.'}</Text>
        ...
        <TouchableOpacity style={s.primaryBtn} onPress={handleSignUp} activeOpacity={0.85} disabled={loading}>
          {loading ? (
            <ActivityIndicator color={Colors.black} />
          ) : (
            <Text style={s.primaryBtnTxt}>{mode === 'signup' ? 'CREATE ACCOUNT' : 'SIGN IN'}</Text>
          )}
        </TouchableOpacity>

        <TouchableOpacity onPress={() => { setMode(mode === 'signup' ? 'signin' : 'signup'); setError(''); }} style={s.switchBtn} activeOpacity={0.7}>
...
const s = StyleSheet.create({
  ...
  content: { paddingHorizontal: 28, paddingTop: 10, paddingBottom: 40, alignItems: 'center' },
  avatar: { width: 80, height: 80, borderRadius: 40, marginBottom: 20 },
  title: { fontSize: 26, fontWeight: '800', color: Colors.primary, textAlign: 'center', marginBottom: 8 },
  subtitle: { fontSize: 15, color: Colors.textSecondary, textAlign: 'center', marginBottom: 24 },
  error: { fontSize: 13, color: '#C47A6C', textAlign: 'center', marginBottom: 16, backgroundColor: '#2A1515', borderRadius: 10, padding: 12, width: '100%' },
  ...
  input: {
    ...
    paddingHorizontal: 18, paddingVertical: 16, fontSize: 15, color: Colors.textPrimary, width: '100%',
  },
  primaryBtn: {
    backgroundColor: Colors.primary, borderRadius: 14, paddingVertical: 18,
    alignItems: 'center', width: '100%', marginBottom: 16,
  },
  primaryBtnTxt: { fontSize: 15, fontWeight: '800', color: Colors.black, letterSpacing: 2 },
  ...
  dividerTxt: { fontSize: 12, color: Colors.textMuted, paddingHorizontal: 16, fontWeight: '600' },
  socialBtn: {
    flexDirection: 'row', alignItems: 'center', backgroundColor: Colors.surface, borderRadius: 14,
    borderWidth: 1, borderColor: Colors.surfaceBorder, paddingVertical: 16, paddingHorizontal: 18,
    width: '100%', marginBottom: 10, opacity: 0.5,
  },
  socialIcon: { fontSize: 18, fontWeight: '700', color: Colors.textPrimary, width: 28 },
  socialTxt: { fontSize: 15, fontWeight: '500', color: Colors.textPrimary, flex: 1 },
  comingSoon: { fontSize: 11, color: Colors.textMuted, fontStyle: 'italic' },
  guestBtn: { marginTop: 10, paddingVertical: 14, width: '100%', alignItems: 'center' },
  guestTxt: { fontSize: 14, color: Colors.textMuted, fontWeight: '500' },
  terms: { fontSize: 11, color: Colors.textMuted, textAlign: 'center', marginTop: 20, lineHeight: 16, paddingHorizontal: 20 },
});