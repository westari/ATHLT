# Deploy: Supabase CV Schema

Run the SQL below to create the `shot_sessions` and `shots` tables.

---

## Steps

1. Go to **supabase.com/dashboard** → select the ATHLT project
   (URL: `https://tvtojlwdpipntkktguck.supabase.co`)

2. Click **SQL Editor** in the left sidebar

3. Click **"New query"**

4. Paste the entire SQL block below

5. Click **"Run"** (or Ctrl+Enter)

6. Verify: go to **Table Editor** and confirm `shot_sessions` and `shots` appear
...
After running, you should see in Table Editor:
- `shot_sessions` — 20 columns including `coach_analysis` (JSONB) and `session_type`
- `shots` — 15 columns including `zone`, `release_angle`, `arc_height`
- RLS enabled on both (lock icon in Table Editor)

The `user_zone_stats` view appears under **Database → Views**.

---

## Notes

- The tables already existed in the schema plan from before CV was scaffolded
- The `coach_analysis` JSONB column stores the full response from `coach-shot-read.js`
- Zone data feeds into plan generation via `buildShootingContextString()` in `lib/shotSync.ts`
